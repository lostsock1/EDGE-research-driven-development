...
**The merge tap can live in the chat without weakening the gate.** The PR gate (`edge-pr-gate.sh`) moves the approval *surface* to a thread button while keeping the approval *authority* with the operator: actions are minted only by the sweep from observed GitHub state, are single-use, and `act` re-verifies the preconditions at execution time (PR still open, every check green, every per-project named context present/pass, current-head reviewer marker eligible, branch still stale) — so a stale button can never merge a red PR. The research agent is under doctrine to never merge or delete branches outside `act`; the coder agents keep their hard `deny` on merges either way. The sweep's second job is entropy control: repos converge to trunk-only because every merged/orphaned branch gets offered for pruning.
